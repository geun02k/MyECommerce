...
## < 주요 기능 >
### 1. 인증/인가
- 회원가입 (SELLER / CUSTOMER 권한 분리)
- JWT 기반 로그인 / 로그아웃
- Spring Security Filter 기반 접근 제어
- 비밀번호 암호화 처리

### 2. 상품 관리 (판매자 권한)
- 상품 등록 (옵션 포함)
- 상품 수정 (설명, 판매 상태)
- 판매 상태 관리 (ON_SALE / DISCONTINUED / DELETION)
- 본인 상품만 수정 가능
...
- [x] 상품 주문
    ...
    - [ ] 3일 뒤 주문 만료(자동 취소) 처리    
          주문과 결제가 동시에 진행되지 않으므로, 결제되지 않은 주문은 자동 취소하면서 재고 원복.
          배치로 10분 주기 실행.   
          일시적 주문상태와 만료여부 불일치가 있을 수 있으나, 의도한 지연된 정합성이므로 괜찮다.
          단, 결제 전 만료일자를 검증이 추가로 필요하다.
- [ ] 상품 결제
    - [ ] PG사 연동 결제 api를 이용한 결제서비스 생성해 결제진행. (PG사 실연동 x)
    - [x] 결제 실패 시 상품주문을 취소처리.     
          주문과 동시에 결제가 이뤄지는 구조가 아니므로 제외.
    - [x] 동시성제어 필요여부확인 후 개발진행.
    - [x] 결제 완료 후 주문상태 '결제완료'로 변경.
    - 참고블로그   
        - 토스 페이먼츠 개발자 센터      
