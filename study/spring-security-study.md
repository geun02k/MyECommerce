...
- 인증(Authentication)   
  서비스의 사용자임을 확인.
- 인가(Authorization)   
  인증된 사용자가 서비스에 접근 시 요청된 자원에 대한 접근 허가.

1. 인증, 인가를 통한 보호기능 제공
   유저 정보 인증, 인증된 유저의 정보 권한을 추출해 요청한 자원에 접근 권한이 있는지 판단해 보안 수행.   
   보안 작업은 까다롭지만 스프링 시큐리티를 이용하면 제공 기능들을 이용해 보다 쉽게 보안작업 진행가능.

2. 스프링 시큐리티는 필터 방식으로 동작.   
   인증, 인가에 대한 처리를 스프링 시큐리티 내부적 필터를 순차적으로 통과하면서 수행.   
   Spring Security 의존성을 추가하게 되면 15개의 **기본 필터들이 자동으로 적용**됨.   
   설정에 따라 필요한 필터가 있고 필요없는 필터가 있을 수 있음.
   이는 **SecurityConfig에서 필요하지 않은 필터들을 끌 수 있음.**

3. HttpSecurity   
   실제 필터를 생성.   
   생성된 필터들은 WebSecurity 클래스를 통해 FilterChainProxy의 인자들로 전달됨.   
   1) 사용자가 요청 
   2) DelegatingFilterProxy가 가장 먼저 그 요청 받음 
   3) FilterChainProxy에게 요청 위임 
   4) 위임받은 요청에 해당하는 SecurityFilterChain이 필터 수행 
   5) 위의 과정 반복...
...
2. Authentication 객체   
   ...
   세션에 Authentication 객체가 들어가는 것은 로그인 되었다는 의미.

1. Spring Security 아키텍처 (Spring 요청 처리과정)
...
1. Authentication 객체 생성   
   사용자가 자격 증명 정보를 제출하면, AbstractAuthenticationProcessingFilter가 Authentication 객체를 생성.
   - AbstractAuthenticationProcessingFilter   
     SecurityFilterChain 객체 반환.
2. Authentication 객체 전달   
   Authentication 객체가 AuthenticationManager에게 전달됨.
3. 인증 실패 경우   
   인증에 실패 시, 로그인 된 유저정보가 저장된 SecurityContextHolder의 값이 지워지고 RememberMeService.joinFail() 메서드가 실행됨. 
   그리고 AuthenticationFailureHandler가 실행됨.
4. 인증 성공 경우   
   인증에 성공하면, SessionAuthenticationStrategy가 새로운 로그인이 되었음을 알리고, 
   Authentication 이 SecurityContextHolder에 저장됨.    
   이후에 SecurityContextPersistenceFilter가 SecurityContext를 HttpSession에 저장하면서 로그인 세션 정보가 저장됨.
   그 뒤로 RememberMeServices.loginSuccess()가 실행되어 
   ApplicationEventPublisher가 InteractiveAuthenticationSuccessEvent를 발생시키고 AuthenticationSuccessHandler가 실행됨.
...
2. @EnableWebSecurity 어노테이션 적용필수.
   - @EnableWebSecurity    
     스프링 시큐리티를 활성화하고 웹 보안 설정 구성에 사용됨.   
     자동으로 스프링 시큐리티 필터 체인을 생성하고 웹 보안 활성화.   
     1) 웹 보안 활성화   
        웹 보안 설정 활성화. 스프링 시큐리티의 필터 체인이 동작하여 요청을 인가하고 인증.
     2) 스프링 시큐리티 구성   
        스프링 시큐리티 구성을 위한 WegbSecurityConfigurer 빈 생성.
        구성 클래스에서 configure() 메서드를 오버라이딩하여 웹 보안 설정 구성가능.   
     3) 다양한 보안 기능 추가   
        ex) 폼 기반 인증, 로그인 페이지 구성, 권한 설정 등 가능.   
...
1. 비밀키 인코딩   
   ...
   - 터미널에서 인코딩된 문자열 생성가능.   
     - 평문키 파일을 생성해 base64로 인코딩된 문자열을 포함한 새 파일 생성.
       ~~~
...
2. application.yml에 환경변수설정 추가
   - 설정한 변수 호출해 JWT 토큰 생성 시 사용가능.
   - spring.jwt.secret-key: '암호화된 비밀키 등록'

3. JWT 서명 검증을 위한 비밀키 생성 (JwtAuthenticationProvider.java)    
   JWT 생성 시 사용할 비밀키 생성.
...
### < JWT 생성/검증/인증 >       
1. JWT 토큰생성 (JwtAuthenticationProvider.java)
   1) JWT   
      사용자 인증 및 데이터 전송을 위한 경량의 안전한 토큰 형식으로 사용됨.
   2) Jwts   
      JWT 생성 및 검증을 도와주는 Java 라이브러리.
      JJWT(Java JWT Library) 라이브러리에서 제공하는 주요 클래스 중 하나로,
   ...
   4) JWT 생성 및 서명 (Creating JWT, Signing JWT)   
...
2. JWT 토큰생성 테스트코드 작성
   - 참조 : JwtAuthenticationProviderTest.successCreateTokenTest()
...
5. Spring Security에서 JWT 토큰을 사용한 인증.
...
6. JWT 토큰과 세션   
   1) JWT 토큰을 사용해 사용자 인증을 진행하는 경우 Session stateless 설정하는 이유.
      - 참고 블로그      
...
1. 사용자 Entity 생성 (Member.java)   
   ...
   1) UserDetails 인터페이스   
      ...
      - Spring Security에서 인증을 처리할 때, 사용자 정보를 제공하는 UserDetails 객체는 
...
2. 사용자 인증 service (MemberService.java)   
   ...
   1) UserDetailsService 인터페이스   
      ...
      - UserDetailsService는 UserDetails 인터페이스를 사용하여 인증된 사용자를 로드하고, 
        AuthenticationManager는 이 정보를 바탕으로 인증을 수행.
...
3. 인증 Filter (JwtAuthenticationFilter.java)   
   ...
   1) OncePerRequestFilter 필터를 상속해 생성.   
      해당 필터를 상속해 정의해주면 모든 요청이 올 때 마다 (한 요청당 한 번) 필터 실행.
   2) doFilterInternal() 메서드 필수구현.
   3) 사용자가 signup, signin api를 호출했을 때 바로 해당 컨트롤러로 요청이 들어오는 것은 아니다.   
   ...
   4) UsernamePasswordAuthenticationToken 클래스  
