...
- Redis
  - 빠른 응답 속도, 실시간 처리가 중요한 경우로 예를 들면 실시간 장바구니 상태를 빠르게 업데이트해야 하는 경우.
  - 장바구니가 비교적 간단한 데이터 구조를 가지고 있고, 영속성이 중요하지 않거나 적당한 백업 전략을 적용할 수 있는 경우.
  - 사용자가 로그인 상태에서 장바구니를 유지해야 할 때와 같이 세션 기반 관리가 필요한 경우.
  > 영속성이 중요하지 않고 세션 관리가 중요한 이커머스 시스템에서는 매우 유용.

- 데이터베이스
  - 데이터의 영속성(데이터를 장기간 보관)이 중요한 경우.
  - 다양한 비즈니스 로직을 처리해야 하거나, 복잡한 쿼리가 중요한 경우.    
    ex) 장바구니 내에서 가격, 할인이 자주 변동
  - 데이터 일관성이 중요한 시스템.    
    ex) 장바구니 외에도 다른 주문 및 결제 관련 데이터를 함께 처리하는 경우.
  > 데이터의 일관성 및 영속성이 보장되는 환경이 필요하다면 데이터베이스를 선택하는 것이 더 나은 선택이 될 수 있음.

- 혼합
  - Redis는 장바구니 데이터를 캐시 용도로 사용하고, 
  - 데이터베이스는 최종적인 영속적 저장소로 사용하는 방식으로 두 기술을 혼합해 사용하는 경우도 많음.

1. 장바구니 기능을 구현 시 데이터베이스와 Redis 선택 
   - 데이터베이스로 구현
     1. 영속성
   ...
   - Redis로 구현
     1. 속도
        ...
        - 장바구니와 같은 빈번하게 변경되는 데이터를 처리할 때 빠른 성능을 보장.
     2. 세션 관리
...
2. 조회성능의 문제라면 상품검색의 경우 api 요청이 가장 많으니 조회 성능이 무엇보다 중요한데, 이는 redis에 저장하지 않고 데이터베이스에 저장하는 이유
   > 상품 검색은 주로 대규모 데이터에 대한 복잡한 쿼리와 정확한 결과를 요구.
   > 단순히 성능만 고려해서 Redis를 사용하는 것이 항상 적합하지 않을 수 있음.
   > 상품 검색 기능에 있어 조회 성능이 매우 중요한 경우,
     데이터베이스를 사용하는 이유는 복잡한 검색 쿼리, 데이터의 일관성 유지, 영속성 보장, 그리고 인덱스 활용 등이 매우 중요한 역할을 하기 때문입니다.
   ...
   1. 상품 데이터의 복잡성
      ...
      - 복잡한 쿼리나 집계 작업이 필요한 상품 검색에는 데이터베이스가 더 유리.
   2. 데이터의 일관성 유지
      - 상품 데이터는 일관성이 매우 중요. 
...
3. 장바구니 기능을 구현하면 로그인 토큰을 redis에 아무런 분리 없이 저장하면 되나?
   > 로그인 토큰과 장바구니 기능을 Redis에 저장하는 경우
     토큰 정보와 장바구니 정보를 같은 Redis 인스턴스에 저장하는 것은 가능하지만 분리를 추천함.
   > 명확한 네이밍 규칙과 데이터 구조를 통해 키 충돌을 방지하고, 관리가 용이하도록 함.
     로그인 토큰과 장바구니 데이터의 TTL을 다르게 설정하여, 유효 기간에 맞는 데이터 삭제 가능.
     보안과 성능을 고려하여 적절한 구조와 정책을 적용가능.
     따라서 **로그인 토큰과 장바구니 데이터를 동일한 Redis 인스턴스에 저장하되
     명확한 네이밍과 구조적인 분리**를 통해 관리하는 것이 가장 효율적이고 안전.

   1. 데이터의 분리
      - 동일한 Redis 인스턴스에 로그인 토큰 정보와 장바구니 정보를 저장가능. 
      - 하지만 **로그인 토큰, 장바구니 정보를 명확히 분리**하는 것이 중요. 
        이렇게 분리하지 않으면, 나중에 **데이터 충돌이나 관리 어려움이 발생가능.**
      > 해결방법
      > - 네임스페이스 활용
      >   - Redis에서는 :, _ 등의 구분자를 사용하여 키를 네임스페이스처럼 관리가능.
      >   - session과 cart 키를 명확히 구분할 수 있어 데이터 충돌을 방지가능.  
      > - Hash 구조 사용
      >   - Redis에서 Hash 구조를 시 한 사용자에 대한 여러 관련 데이터를 하나의 엔티티로 관리가능.
      >   - 로그인 정보와 장바구니 정보를 같은 Hash에서 관리할 수도 있지만, 구분된 별도의 Hash로 관리하는 것이 좋음.
      ~~~
   ...
   2. TTL (Time To Live) 관리
      - 로그인 토큰은 보통 유효 기간이 정해져 있지만,
        장바구니 정보는 사용자가 장바구니를 비우거나 특정 시간 동안 활동이 없으면 자동으로 삭제되도록 설정가능. 
        이때 TTL을 설정해주는 것이 유용합니다.
      - 로그인 토큰의 TTL
   ...
   4. 데이터 유지 기간의 차이
      ...
      - 로그인 토큰, 장바구니는 TTL 값을 별도로 설정하여 각기 다른 관리 방식을 적용해야 합니다.
   5. 장바구니와 세션 데이터의 관계
...
4. 장바구니에서 상품 품절
   - 장바구니에 담긴 상품을 조회할 때마다 실시간으로 판매 가능 수량을 확인하고, 품절 상태를 업데이트하는 방식으로 구현됨.
   > 품절된 상품을 장바구니에서 처리하는 방법
     **판매 가능 수량을 실시간으로 조회**하여, **장바구니에 담을 때나 조회할 때 품절 상태를 반영**하는 방식으로 구현가능.
     **Redis의 Hash 구조를 활용하여 장바구니 데이터를 관리**하고, 상품의 재고 상태를 Redis에서 직접 조회하여 품절 상품을 처리하는 방법이 가장 효율적입니다.
   > 장바구니에 상품을 추가할 때 재고를 확인하여 품절된 상품은 추가하지 않거나 품절 상태로 처리합니다.
   > 장바구니를 조회할 때 품절된 상품을 자동으로 확인하고 상태를 업데이트합니다.
   > 필요하다면 Redis Pub/Sub을 활용하여 실시간으로 품절 상품을 동기화할 수 있습니다.
   ...
   5. Redis 상품판매가능수량 관리
      ...
      > 데이터베이스에서 판매 가능 수량을 조회하고, 이를 Redis에 캐시.
      > 장바구니에 상품을 추가할 때마다 Redis에서 판매 가능 수량을 조회하고 품절 여부를 확인.
      > 판매 시, Redis와 데이터베이스에서 재고를 차감하고 동기화.
      > Redis와 데이터베이스의 동기화를 주기적으로 관리하거나, TTL을 사용하여 자동 갱신.
      > 이 방식으로 Redis와 데이터베이스를 적절히 활용하면, 장바구니 기능을 구현할 때 성능과 일관성을 유지할 수 있습니다.
      
      > 기본 프로세스
      > - 판매 가능 수량이 변경되면 (예: 상품이 팔리거나 재고가 추가될 때) 이를 데이터베이스에 반영하고, Redis에 해당 값을 업데이트.
      > - 장바구니에 상품을 추가하거나 조회할 때, Redis에서 판매 가능 수량을 조회.
      > - Redis에서 판매 가능 수량을 찾을 수 없거나 오래된 데이터일 경우, 데이터베이스에서 다시 조회하여 Redis에 최신 값을 저장.
