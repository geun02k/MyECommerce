...
9. 상품수정 시 동작하는 쿼리의 변환과정
10. 조회성능에 대한 고민과 조회쿼리 최적화
...
2. 품절여부를 출력하기 위한 구조
   - CART 네임스페이스: 유저 장바구니 정보
   - STOCK 네임스페이스: 옵션별 재고 수량
   - 상품 등록/수정 시 → Redis STOCK 갱신
   - 장바구니 조회 시 → DB 접근 없이 Redis만 조회
   - 재고 0이면 → 품절 플래그 → 프론트 disable

3. Redis로 품절 여부 관리하는 이유
   1) 빈번한 조회 발생    
      장바구니 유저 진입 시 거의 무조건 호출하므로, 조회는 빈번하게 발생한다.
      특히 페이지 이동, 새로고침, 옵션 병경 시 반복 호출된다.
      여기서 매범 DB 재고 테이블을 조인하게 되면 DB 부하가 급증하므로, 캐시를 사용하지 않으면 성능에서 차이가 난다.
      따라서 장바구니/목록/상세 화면은 캐시가 우선된다.
   2) 강한 정합성 불필요    
      품절 여부는 장바구니에서 대충 살 수 있냐 없냐 정도를 보여주면 되므로, 1~2초 늦게 반영되어도 비즈니스적으로는 문제가 없다.
      따라서 정확할 필요가 없고, 빠르고 안정적인 응답이 중요하다.
